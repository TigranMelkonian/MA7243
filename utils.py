...
def dataset_constructor(video_loc, img_folder, train_frames, dataset_type, ground_truth_speed, CLEAN_DATA_PATH):
    """
    Creates raw data meta csv file from video input including path to frame img 
    and its respective ground truth speed value provided by Comma.ai
    """
    meta_dict = {}
    ...
    for idx, frame in enumerate(tqdm(cap)):    
        ...
        frame_speed = float('NaN') if dataset_type == 'test' else ground_truth_speed[idx]
...
def train_valid_split(dframe, seed_val):
    ...
    generates a uniform random variable 0->9, gives 15% chance to append to valid data, otherwise train_data
...
def train_valid_test_split(dframe, seed_val):
    """
    Randomly shuffle pairs of rows in the dataframe, separates train and validation data
    generates a uniform random variable 0->9, gives 10% chance to append to valid data, 10% chance to append to test data, 
    otherwise appends to train data
    return tuple (train_data, valid_data, test_data) dataframes
    """
    train_data = pd.DataFrame()
    valid_data = pd.DataFrame()
    test_data = pd.DataFrame()
...
def predict_speed(data, model):
    """
    Function for predicting speed after a model has been trained. Requires testing data or validation data as input
    """
...
def preprocess_image(image):
    ...
    Process: 
             1) Cropping out black spots within video frames
